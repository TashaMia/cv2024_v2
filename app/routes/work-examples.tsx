import VideoComponent from "~/work-example/components/VideoComponent";
import { videoBd } from "~/work-example/videodb";
...
export default function WorkExamples() {
  return (
    <div className="flex w-full justify-center items-center  flex-col gap-8">
      {videoBd.map((video, index) => {
        return <VideoComponent video={video} index={index} />;
      })}
    </div>
  );
}
